...
const SingleCourse = () => {
  ...
  return (
    ...
    {
      getdata.courses.map((ele)=>{
        return(
          <div className="flex flex-col gap-16">
            {ele.categories.map((categories)=>{
              return(
               <div className=''>
                 ...
                 <div className="grid grid-cols-4 gap-7">
                 {
                  categories.videos.map((video)=>{
                    return(
                        <Link to={`/course/${video.videoId}`} key={video.videoId} className="mainCard pb-[60px] relative rounded-2xl bg-white border-2 border-yellow-500 overflow-hidden">
