               ...
               <div className='' key={Math.random()}>
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7">
                 {
                  categories.videos.map((video)=>{
                    return(
                        <Link to={`/course/${getdata.courseId}/${categories.categoryName}/${video.videoId}`} key={video.videoId} className="mainCard pb-[60px] relative rounded-2xl bg-white border-2 border-yellow-500 overflow-hidden">
